...
__global__ void compare_lsh_kernel(unsigned int *buf, unsigned char* result, int line_num, int num_hash, int th) {
    ...
    if(dx<lim1 && dy <lim2 ) result[(size_t)(bias1+dx)*line_num+(bias2+dy)]=cnt>th;
}
    ...
    for(int b_id=rank; b_id<b; b_id+=size) {
        ...
        for(int key=0; key<num_key; key+=C) {
            ...
            for(int i=0; i<num_file; i++) {
                ...
                int fs = file_size[i];                // Get the size of the file
            ...
            for(int c=0; c<C; c++) {
                ...
                dim3 blockSize((cnt[c] + block - 1) / block, (cnt[c] + block - 1) / block);
                dim3 numBlocks(THREAD_NUM);
                compare_lsh_kernel<<<blockSize, numBlocks>>>(cuda_buf1, cuda_cmp_result, cnt[c], num_hash, th);
